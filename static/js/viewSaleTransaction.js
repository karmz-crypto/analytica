...
function getSaleData(event){
    //console.log(event.target);
    let id = event.target.dataset.id;
   // console.log(id);   
    let url = `/api/transaction/sale/${id}`;
    ...
    fetch(url).then(res=>{
        ...
        res.json().then(data=>{ //console.log(data);
          ...
          data.saleProductInfo.forEach(product=>{
              ...
              paraEl.classList.add('p-2','my-1','bg-light','border','border-1','border-success','rounded','text-dark');
              paraEl.innerHTML =  `
              ...
              Sale Tunch : <span class="mx-3 fw-bold productTunch">${product.saleTunch}</span> % <br>
              Sale Weight : <span class="mx-3 fw-bold productWeight">${product.saleWeight}</span> Grms <br>
              Labour/Kg : <span class="mx-3 fw-bold productLabour">${product.saleLabourPerKg}</span> Rs <br>`;
            ...
            liElement_1.appendChild(document.createTextNode(`Fine Silver : ${data.saleSilver} Grms`));
            ...
            liElement_2.appendChild(document.createTextNode(`Purchase Cash : ${data.saleCash} Rupees`));
...
function searchSaleTransaction(event){
...
function postDataToSearchSale(searchObj){ console.log(searchObj);
    if(!searchObj.hasOwnProperty('searchDateEnd')){ console.log('search name');
        let search = 'searchName';
        let url = `/api/transaction/sale/${search}`;
        fetch(url,{
            method:"POST",
...
function insertToDomSale(data){ console.log(data);
    let makePaymentUrl = '/client/clientName';
    document.querySelector('.recentSaleTransactionDiv').classList.add('d-none');
    var searchEl = document.querySelector('.searchSaleTransactionDiv');
    searchEl.innerHTML = `Client Name : <span class="clientName mx-2 fw-bold"></span><br>
    <p class="fw-bold">Net Fine Silver :<span class="mx-2 tSaleSilver"></span><br>Net Cash Amt : <span class="mx-2 tSaleCash"></span><br></p>
    ...
    data.forEach(data=>{
        ...
        var index = 1;
        var totalSaleSilver = 0;
        var totalSaleCash = 0;
        data.salesProduct.forEach(purchase=>{
            ...
            spanFineSilver.appendChild(document.createTextNode(`Fine Silver : ${purchase.saleSilver} Grms`));
            ...
            spanPurchaseCash.appendChild(document.createTextNode(`Labour Cash : ${purchase.saleCash} Rupees`));
            liElement_1.appendChild(spanPurchaseCash);
            searchEl.appendChild(liElement_1);
            let productIndex = 1;
            totalSaleCash += purchase.saleCash;
            totalSaleSilver += purchase.saleSilver;
            purchase.saleProductInfo.forEach(product=>{
                ...
                spanPurchaseWeight.appendChild(document.createTextNode(`Purchase Weight : ${product.saleWeight} Grms`));
                ...
                spanPurchaseTunch.appendChild(document.createTextNode(`Purchase Tunch : ${product.saleTunch} %`));
                liElement_2.appendChild(spanPurchaseTunch);
                liElement_2.appendChild(document.createElement('br'));
                let spanLabour = document.createElement('span');
                spanLabour.classList.add('fw-bold');
                spanLabour.appendChild(document.createTextNode(`Labour : ${product.saleLabourPerKg} Rs/Kg`));
       ...
       document.querySelector('.tSaleSilver').innerHTML = `${totalSaleSilver} Grms`;
       document.querySelector('.tSaleCash').innerHTML = `${totalSaleCash} Rupees` ;
    });
